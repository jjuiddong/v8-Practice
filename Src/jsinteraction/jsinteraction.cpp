//
// 2022-01-25, jjuiddong
// v8, javascript <-> c++ communication sample
// nuget package, https://github.com/pmed/v8-nuget
//
...
void SetVar(const v8::FunctionCallbackInfo<v8::Value>& args);
void GetVar(const v8::FunctionCallbackInfo<v8::Value>& args);
...
v8::Local<v8::Context> CreateShellContext(v8::Isolate* isolate) {
	...
	global->Set(isolate, "test", v8::FunctionTemplate::New(isolate, TestFunc));
	// Bind the 'getVar(variable name) function
	global->Set(isolate, "getvar", v8::FunctionTemplate::New(isolate, GetVar));
	// Bind the 'setVar(variable name, value) function
	global->Set(isolate, "setvar", v8::FunctionTemplate::New(isolate, SetVar));
...
void TestFunc(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	v8::String::Utf8Value file(args.GetIsolate(), args[0]);
	if (*file == NULL) {
		args.GetIsolate()->ThrowError("Error Test argument");
		return;
	}
...
void GetVar(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	v8::String::Utf8Value varName(args.GetIsolate(), args[0]);
	const char* cstr = ToCString(varName);
	if (std::string(cstr) == "val1")
	{
		//v8::Local<v8::String> result;
		//v8::MaybeLocal<v8::String> var = v8::String::NewFromUtf8(
		//	args.GetIsolate(), "test1", v8::NewStringType::kNormal, 5);
		//var.ToLocal(&result);
		//args.GetReturnValue().Set(result);

		//v8::Local<v8::Number> result = v8::Integer::New(args.GetIsolate(), 1);
		//args.GetReturnValue().Set(result);

		v8::Local<v8::Number> result = v8::Number::New(args.GetIsolate(), 0.1);
		args.GetReturnValue().Set(result);

		// 
		//v8::Local<v8::Number> result;
		//v8::MaybeLocal<v8::Number> var = v8::Number::IntegerValue(
		//	args.GetIsolate(), "test1", v8::NewStringType::kNormal, 5);
		//var.ToLocal(&result);
		//args.GetReturnValue().Set(result);

	}
	if (std::string(cstr) == "val2")
	{
		//v8::Local<v8::String> result;
		//v8::MaybeLocal<v8::String> var = v8::String::NewFromUtf8(
		//	args.GetIsolate(), "test2", v8::NewStringType::kNormal, 5);
		//var.ToLocal(&result);
		//args.GetReturnValue().Set(result);

		//v8::Local<v8::Number> result = v8::Integer::New(args.GetIsolate(), 2);
		//args.GetReturnValue().Set(result);

		v8::Local<v8::Number> result = v8::Number::New(args.GetIsolate(), 0.2);
		args.GetReturnValue().Set(result);
	}
...
int RunMain(v8::Isolate* isolate, v8::Platform* platform, int argc,
	...
	ExecuteString(isolate
		, v8::String::NewFromUtf8Literal(isolate, "testFunction()") 
		, v8::String::NewFromUtf8Literal(isolate, "unnamed")
		, true, true);
